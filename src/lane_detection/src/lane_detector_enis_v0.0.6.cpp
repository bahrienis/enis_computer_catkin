...
std::string filename = "/home/enis/Schreibtisch/deneme2/frame" + std::to_string(count) + ".jpg";
...
for( size_t i = 0; i < lines.size(); i++ )
{
  ...
  vector<Vec4i> lines;
  HoughLinesP(dst, lines, 1, CV_PI/180, 10, 10, 50 );
  for( size_t i = 0; i < lines.size(); i++ )
  {
    Vec4i l = lines[i];
 //   if(l[0]<=l[2] && l[1]>=l[3]){
...
vector<Vec2f> lines1;
HoughLines(dst1, lines1, 1, CV_PI/180, 50, 0, 0 );
...
*/
  ...
  vector<Vec4i> lines1;
  HoughLinesP(dst1, lines1, 1, CV_PI/180, 10, 10, 50);
  for( size_t i1 = 0; i1 < lines1.size(); i1++ )
  {
    Vec4i l1 = lines1[i1];
    line( cdst1, Point(l1[0], l1[1]), Point(l1[2], l1[3]), Scalar(0,0,255), 3, CV_AA);
//  circle(cdst1, Point(l1[0], l1[1]), 1, Scalar(0,0,255), 3, CV_AA, 0);
//  circle(cdst1, Point(l1[2], l1[3]), 1, Scalar(0,0,255), 3, CV_AA, 0);
//cout<< Point(l1[0], l1[1])<<endl;
